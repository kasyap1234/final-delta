...
from src.backtest.market.order_book import SimulatedOrderBook, OrderBookConfig
...
class DynamicSlippageConfig:
    ...
    # Enable dynamic slippage (vs fixed)
    enable_dynamic_slippage: bool = True

    # Base slippage in bps when volatility is normal
    base_slippage_bps: float = 5.0

    # Minimum slippage in bps (floor)
    min_slippage_bps: float = 2.0

    # Maximum slippage in bps (cap)
    max_slippage_bps: float = 50.0

    # Volatility thresholds (as decimal, e.g., 0.02 = 2%)
    low_volatility_threshold: float = 0.01  # 1% - below this, use min slippage
    high_volatility_threshold: float = 0.05  # 5% - above this, use max slippage

    # Volatility lookback period for ATR calculation
    volatility_lookback: int = 14

    # Size impact multiplier (additional bps per 1% of volume)
...
class SimulatorConfig:
    ...
    post_only_reject_probability: float = 0.05  # residual rejection/race risk
    ...
    base_fill_probability: float = 0.70
    distance_penalty_factor: float = 0.8  # Penalty per 1% from mid
    size_penalty_factor: float = 0.5  # Penalty per 10% of volume

    # Market order slippage (used as fallback when dynamic is disabled)
    market_order_slippage_bps: float = 10.0  # 10 bps slippage
...
class BacktestOrderSimulator:
    ...
    def __init__(
        ...
        # Volatility tracking for dynamic slippage
        self._volatility_history: Dict[str, List[float]] = {}  # symbol -> [volatility values]
        self._candle_history: Dict[str, List[OHLCV]] = {}  # symbol -> [recent candles]
        ...
        self._stats = {
            ...
            "latency_count": 0,
            "dynamic_slippage_adjustments": 0,
        }
        ...
        logger.info(
            "BacktestOrderSimulator initialized with latency, order book, and dynamic slippage support"
        )
    ...
    def submit_order(
        self, order: Order, current_time: Optional[datetime] = None
    ) -> Order:
        """
        ...
        if self.config.enable_latency and current_time:
            event = self.latency_model.schedule_event(
                LatencyType.ORDER_SUBMIT, current_time, metadata={"order_id": order.id}
            )
        ...
        logger.debug(
            f"Order submitted: {order.id} {order.side} {order.amount} @ {order.price}"
        )
        return order
    ...
    def cancel_order(
        self, order_id: str, current_time: Optional[datetime] = None
    ) -> Optional[Order]:
        """
        Cancel an order.
        ...
        if self.config.enable_latency and current_time:
            event = self.latency_model.schedule_event(
                LatencyType.ORDER_CANCEL, current_time, metadata={"order_id": order_id}
            )
    ...
    def process_orders(
        self, candle: OHLCV, current_time: Optional[datetime] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _update_order_book(self, candle: OHLCV) -> None:
        ...
        if candle.symbol not in self._order_books:
            self._order_books[candle.symbol] = SimulatedOrderBook(
                candle.symbol, self.config.order_book_config
            )

        # Calculate volatility from candle
        volatility = (
            (candle.high - candle.low) / candle.close if candle.close > 0 else 0.02
        )

        self._order_books[candle.symbol].update_from_candle(
            candle, volatility=volatility, timestamp=candle.timestamp
        )
    ...
    def _is_tif_expired(self, order: Order, current_time: datetime) -> bool:
        ...
        if order.time_in_force == "IOC":
            if order.order_type == "market":
                # Market IOC should execute immediately; do not expire pre-fill.
                return False
    ...
    def _process_market_order(
        self, order: Order, candle: OHLCV, current_time: datetime
        ...
        if order_book:
            ...
            fill_price, filled_amount, is_complete = order_book.calculate_fill_price(
                order.side, order.remaining, allow_partial=True
            )
        ...
        fee = self._calculate_fee(order, fill_price, filled_amount, is_maker=False)
    ...
    def _process_limit_order(
        ...
        Returns:
            Fill information if filled
        """
        # Post-only rejection is a placement-time check.
        # Once an order is resting, it should not get repeatedly rejected.
        if (
            order.post_only
            and order.status == OrderStatus.PENDING
            and self._should_reject_post_only(order, candle)
        ):
        ...
        # Fill is certain when price conditions are met (reflects real exchange behavior)
        ...
        if order_book:
            fill_price, actual_fill_amount, is_complete = (
                order_book.calculate_fill_price(
                    order.side,
                    fill_amount,
        ...
        fee = self._calculate_fee(order, fill_price, fill_amount, is_maker=is_maker)
        ...
        if order.average_fill_price == 0:
            ...
            total_value = (order.average_fill_price * previous_filled) + (
    ...
    def _should_reject_post_only(self, order: Order, candle: OHLCV) -> bool:
        ...
        # Post-only rejection only happens if order would cross the spread
        # Random rejection removed - real exchanges don't randomly reject valid post-only orders
        return False
    ...
    def _calculate_fill_probability(self, order: Order, candle: OHLCV) -> float:
        ...
        if order.side == "buy":
            ...
            if order.price < mid_price:
                distance_factor = 1.0 - (
                    distance_pct * self.config.distance_penalty_factor * 100
                )
            else:
                # Above mid = lower probability
                distance_factor = 0.7 - (
                    distance_pct * self.config.distance_penalty_factor * 200
                )
        else:  # sell
    ...
    def _calculate_fill_amount(self, order: Order, candle: OHLCV) -> float:
        ...
        if candle.volume > 0:
            ...
            # Deterministic fill: 100% if order <= 10% of candle volume, otherwise proportional
            # This matches real exchange behavior where small orders fill completely
            size_ratio = remaining / candle.volume
            if size_ratio <= 0.10:
                # Small orders fill completely when price conditions are met
                max_fill = remaining
            else:
                # Large orders fill proportionally to available liquidity
                max_fill = min(remaining, available_liquidity)
    ...
    def _calculate_dynamic_slippage_bps(
        self, order: Order, candle: OHLCV
    ) -> float:
        """
        Calculate dynamic slippage based on market volatility and order size.

        Uses a volatility-adjusted model where:
        - Low volatility: slippage near minimum
        - High volatility: slippage scales up to maximum
        - Large orders relative to volume: additional size impact
        ...
        """
        slippage_config = self.config.dynamic_slippage_config

        # If dynamic slippage is disabled, use fixed value
        if not slippage_config.enable_dynamic_slippage:
            return self.config.market_order_slippage_bps

        # Calculate current volatility from candle (high-low range as % of close)
        if candle.close > 0:
            candle_volatility = (candle.high - candle.low) / candle.close
        else:
            candle_volatility = 0.02  # Default 2%

        # Update volatility history for this symbol
        symbol = candle.symbol
        if symbol not in self._volatility_history:
            self._volatility_history[symbol] = []
        self._volatility_history[symbol].append(candle_volatility)

        # Keep only recent history
        lookback = slippage_config.volatility_lookback
        if len(self._volatility_history[symbol]) > lookback:
            self._volatility_history[symbol] = self._volatility_history[symbol][-lookback:]

        # Calculate average volatility
        avg_volatility = (
            sum(self._volatility_history[symbol]) / len(self._volatility_history[symbol])
            if self._volatility_history[symbol]
            else candle_volatility
        )

        # Calculate base slippage based on volatility regime
        low_threshold = slippage_config.low_volatility_threshold
        high_threshold = slippage_config.high_volatility_threshold

        if avg_volatility <= low_threshold:
            # Low volatility regime - use minimum slippage
            volatility_slippage = slippage_config.min_slippage_bps
        elif avg_volatility >= high_threshold:
            # High volatility regime - use maximum slippage
            volatility_slippage = slippage_config.max_slippage_bps
        else:
            # Normal volatility - interpolate between base and max
            # Linear interpolation on log scale for smoother transitions
            vol_ratio = (avg_volatility - low_threshold) / (high_threshold - low_threshold)
            base = slippage_config.base_slippage_bps
            max_slip = slippage_config.max_slippage_bps
            volatility_slippage = base + vol_ratio * (max_slip - base)

        # Add size impact for large orders
        size_impact_bps = 0.0
        if candle.volume > 0 and order.amount > 0:
            size_ratio = order.amount / candle.volume
            # Additional slippage proportional to order size as % of volume
            size_impact_bps = size_ratio * 100 * slippage_config.size_impact_bps_per_percent

        # Combine volatility and size impacts
        total_slippage = volatility_slippage + size_impact_bps

        # Clamp to configured bounds
        total_slippage = max(
            slippage_config.min_slippage_bps,
            min(slippage_config.max_slippage_bps, total_slippage)
        )

        # Track statistics
        self._stats["dynamic_slippage_adjustments"] += 1

        logger.debug(
            f"Dynamic slippage for {symbol}: {total_slippage:.2f} bps "
            f"(vol={avg_volatility:.4f}, size_impact={size_impact_bps:.2f} bps)"
        )
    ...
    def _calculate_market_fill_price(self, order: Order, candle: OHLCV) -> float:
        """
        Calculate fill price for market order with dynamic volatility-based slippage.

        Slippage is calculated based on:
        1. Current market volatility (candle range)
        2. Historical volatility (recent average)
        3. Order size relative to candle volume
        ...
        slippage_bps = self._calculate_dynamic_slippage_bps(order, candle)
        slippage_pct = slippage_bps / 10000

        # Track slippage for statistics
        self._stats["total_slippage"] += slippage_bps
    ...
    def _calculate_limit_fill_price(
        self, order: Order, candle: OHLCV, fill_amount: float
    ) -> float:
        """
    ...
    def _calculate_fee(
        ...
        Returns:
            Fee amount
        """
        trade_value = fill_amount * fill_price

        if is_maker:
            fee_pct = self.config.maker_fee_rate
        else:
            fee_pct = self.config.taker_fee_rate
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats["pending_orders"] = sum(
            1 for o in self._orders.values() if o.status == OrderStatus.PENDING
        )
        stats["partially_filled"] = sum(
