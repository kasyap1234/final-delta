...
class AccountState:
    ...
    def __init__(
        ...
        logger.info(f"AccountState initialized: balance={initial_balance} {currency}")

    def set_max_leverage(self, max_leverage: float) -> None:
        """Set the maximum leverage for margin calculations."""
    ...
    def get_fee_history(
        ...
        if end_time:
            records = [
                r for r in records if datetime.fromisoformat(r["timestamp"]) <= end_time
            ]
    ...
    def _open_position(
        ...
        margin_required = position_value / self._max_leverage

        self._used_balance += margin_required
        self._free_balance -= margin_required
    ...
    def _close_position(
        self, position_id: str, exit_price: float, timestamp: datetime, exit_fees: float
    ) -> Optional[Position]:
        """
        ...
        margin_released = (position.size * position.entry_price) / self._max_leverage
        self._used_balance -= margin_released
        self._free_balance += margin_released + realized_pnl - exit_fees
    ...
    def open_position(
        ...
        if position_value > self._free_balance:
            logger.warning(
                f"Insufficient balance to open position: {position_value:.2f} > {self._free_balance:.2f}"
            )
            return False
