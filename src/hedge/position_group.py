"""Position group module for tracking original positions and their hedges.
...
P&L tracking and status management.
"""
...
class HedgeStatus(Enum):
    ...
    NO_HEDGE = "no_hedge"           # No hedge opened yet
    HEDGE_ACTIVE = "hedge_active"   # Hedge position(s) active
    HEDGE_PROFIT = "hedge_profit"   # Hedge in profit (can take profit)
    CLOSING = "closing"             # In process of closing all positions
...
class OriginalPosition:
    ...
    def get_stop_loss_distance(self) -> float:
        ...
        Returns:
            Absolute stop-loss distance in price terms
        """
    ...
    def get_loss_percentage_of_sl(self) -> float:
        ...
        Returns:
            Loss as percentage of SL distance (0.0 = no loss, 1.0 = at SL)
        """
...
class PositionGroup:
    ...
    def __init__(
        ...
        # Track next hedge trigger level
    ...
    def add_hedge(self, hedge: HedgePosition) -> None:
        ...
        Args:
            hedge: The hedge position to add
        """
    ...
    def remove_hedge(self, hedge_id: str) -> Optional[HedgePosition]:
        ...
        Args:
            hedge_id: ID of the hedge to remove
            
        Returns:
            The removed hedge position or None if not found
        """
    ...
    def should_trigger_hedge(self) -> bool:
        ...
        A hedge is triggered when loss exceeds the next trigger level
        (e.g., 50% of SL, then 100%, then 150%, etc.)
    ...
    def check_hedge_profit_targets(self) -> List[HedgePosition]:
        ...
        Returns:
            List of hedge positions that should be closed for profit
        """
        profitable_hedges = []
        ...
        for hedge in self.hedges.values():
            ...
            # Calculate risk amount for this hedge (entry to stop distance)
            if hedge.stop_loss:
                risk_amount = abs(hedge.entry_price - hedge.stop_loss) * hedge.size
            else:
                # Use original position's SL distance as proxy
                risk_amount = self.original.get_stop_loss_distance() * hedge.size
            
            # Check if profit >= target ratio * risk
    ...
    def should_close_all(self) -> bool:
        ...
        All positions should close when total P&L >= 0 (breakeven or profit)
    ...
    def get_hedge_direction(self) -> str:
        ...
        Hedge direction is opposite to original position.
        
        Returns:
            'short' if original is long, 'long' if original is short
        """
    ...
    def close_group(self) -> Dict[str, Any]:
        ...
        Returns:
            Summary of the group's final state
        """
