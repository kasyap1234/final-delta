...
This module provides the BacktestHedgeManager class for monitoring positions,
detecting hedge triggers, managing position groups, and executing
hedge strategies with profit taking and re-hedging logic in backtest environment.
"""
...
class BacktestHedgeManager:
    ...
    def __init__(
        ...
        Args:
            hedge_executor: BacktestHedgeExecutor for executing hedges
    ...
    async def close_hedge(
        ...
        try:
            ...
            if order.get('id'):
                ...
                logger.error(f"Failed to close hedge {hedge_position_id}: {error_msg}")
                return HedgeCloseResult(
                    success=False,
                    hedge_id=hedge_position_id,
                    realized_pnl=0.0,
    ...
    async def process_candle(
        ...
        This method should be called by the backtest engine on each candle
        to check for hedge triggers and manage hedge positions.
        ...
        for group in self.get_active_groups():
            ...
            trigger_result = self.check_hedge_trigger({
                ...
                'current_price': current_prices.get(group.original.symbol, group.original.current_price),
